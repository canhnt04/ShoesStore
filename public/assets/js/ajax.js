  ...
  function logOut(url) {
    $.ajax({
      ...
      success: function (data) {
        ...
        window.location.href = "Route.php?page=Product&action=showList&pageNumber=1"; // Trick bẩn reload trang
        history.pushState({}, "", "Route.php?page=Product&action=showList&pageNumber=1");
      },
      error: function (xhr, ajaxOptions, thrownError) {
        var errorMessage = JSON.parse(xhr.responseText);
        alert(errorMessage.message);
